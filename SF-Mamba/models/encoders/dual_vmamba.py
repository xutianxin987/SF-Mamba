...
from models.encoders.vmamba import Backbone_VSSM, CrossMambaFusionBlock, ConcatMambaFusionBlock,LayerNorm2d
...
class RGBXTransformer(nn.Module):
    def __init__(self,
                 ...
                 depths=[2, 2, 27, 1],  # [2,2,27,2] for vmamba small
                 dims=96,
                 pretrained=None,
                 mlp_ratio=4.0,
    ...
    def forward_features(self, x_A, x_B):
        ...
        for i in range(4):
            if self.ape:
                ...
                out_A = outs_A[i]
                out_B = outs_B[i]
            # print(f"[channel_attn_mamba] input A shape: {out_A.shape}, permuted shape: {out_A.permute(0, 2, 3, 1).shape}")
